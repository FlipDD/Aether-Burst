...
public class PlayerInputController : MonoBehaviour 
{
        ...
        switch (state)
		{
			case State.Moving:
				...
				if (movement != Vector3.zero) 
                {
                    ...
                    // if (!currentState.IsName("Jumping") && rbdg.velocity.y == 0)
                    if (isGrounded)
                ...
                if (Input.GetButtonDown(Jump) && !isGrounded && hasJumped && uiBar.GetBreathValue() > 18 && !uiBar.isChanging && !playerMovement.isTouching)
                {
                    ...
                    // playerMovement.DoubleJump(new Vector3(movement.x, 1f, movement.z));
                    hasJumped = false;
                }
    ...
    internal void Dash() => playerMovement.Dash(transform.forward);
    ...
    void TargetsAndBoulders()
    {
        ...
        // if (Input.GetButtonUp(LeftClick) && (attacktimer > 2.5f && attacktimer < 2.99f) && !state.IsName("combo1") && !state.IsName("combo2") && !state.IsName("combo3"))
        if (Input.GetButtonUp(LeftClick))
        {
            if (state.IsName("RunningTree") && (attacktimer > 2.5f && attacktimer < 2.99f) && !comboCooldown)
            {
